...
class SDE(abc.ABC):
  ...
  def reverse(self, score_fn, probability_flow=False):
    ...
    class RSDE(self.__class__):
      ...
      def sde(self, feature, x, flags, t, pe, is_adj=True):
        """Create the drift and diffusion functions for the reverse SDE/ODE."""
        drift, diffusion = sde_fn(x, t) if is_adj else sde_fn(feature, t)
        score = score_fn(feature, x, flags, t, pe)
      ...
      def discretize(self, feature, x, flags, t, pe, is_adj=True):
...
class VPSDE(SDE):
  def __init__(self, beta_min=0.1, beta_max=20, N=1000, endtime=1, sym=False):
    ...
    """
    super().__init__(N, endtime, sym=sym)
    self.beta_0 = beta_min
    self.beta_1 = beta_max
    self.N = N
    self.discrete_betas = torch.linspace(beta_min * endtime / N, (beta_min + (beta_max - beta_min) * endtime) / N, N)
...
class VESDE(SDE):
  def __init__(self, sigma_min=0.01, sigma_max=50, N=1000, endtime=1, sym=False):
